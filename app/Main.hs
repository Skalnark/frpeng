...
bouncePaddle (gs, key) = let x1       = fst $ p1Pos gs
                             y1       = snd $ p1Pos gs
                             x2       = fst $ p2Pos gs
                             y2       = snd $ p2Pos gs
                             w        = paddleWidth / 2.0
                             ...
                             c2  = y <= y2 + h && y >= y2 - h && x <= x2 && x >= x2 - w
                             v   = if c1 || c2 then (-v1, v2) else (v1, v2)
                             --v = if c1 then (0.0, 0.0) else (v1, v2)
                          in (gs{bVel = v}, key)
...
reset = arr $ \(gs, key) -> (gs{bPos = (0.0, 0.0), bVel = (7.0, -7.0)}, key)



update :: SF (GameState, Key) (GameState, Key)
update = ballScript >>> bounceBall >>> bouncePaddle ^>> controlP1 >>> controlP2
...
render gs = pictures [ ball gs
                     ...
                     , rendP1 gs
                     , rendP2 gs
                     --, translate (0.0, 0.0) $ Color white $ rectangleSolid 30 30
                     ]
