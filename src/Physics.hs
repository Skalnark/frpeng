module Physics (sumVec, dProd, move, translate, sProd, distance, colliding, screenBounce, fVec) where
...
colliding (Rec (x1, y1) r1 r2) (Rec (x2, y2) s1 s2) =
  let p = x1 + r1 >= x2 && x2 + s1 >= x1
      q = y1 + r2 >= y2 && y2 + s2 >= y1
   in p && q
